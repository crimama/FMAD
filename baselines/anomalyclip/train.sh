...
for i in "${!depth[@]}";do
    ...
    ## train on the VisA dataset
        ...
        CUDA_VISIBLE_DEVICES=${device} python train.py --dataset visa --train_data_path /Volume/VAD/Data/VISA \
        --save_path ${save_dir} \
...
for i in "${!depth[@]}";do
    ...
    ## train on the VisA dataset
        ...
        save_dir=./checkpoints/${base_dir}/
        CUDA_VISIBLE_DEVICES=${device} python train.py --dataset mvtec --train_data_path /Volume/VAD/Data/MVTecAD \
        --save_path ${save_dir} \
        --features_list 24 --image_size 518  --batch_size 8 --print_freq 1 \
        --epoch 15 --save_freq 1 --depth ${depth[i]} --n_ctx ${n_ctx[j]} --t_n_ctx ${t_n_ctx[0]}
    wait
